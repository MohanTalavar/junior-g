...
import com.juniorg.utils.UpdateUtils;
...
public class StudentServiceImpl implements IStudentService {
    ...
    public Student getStudentDetails(String studentRollNo) {
        ...
        Student persistentStud = studRepo.findByRollNumber(studentRollNo)
                .orElseThrow(() -> {
                    log.warn("Student not found with roll no: {}", studentRollNo);
...
//			persistentCourse.getStudents().add(stud);// which link are we establishing here ? c-->s 
    ...
    @Override
    public String cancelStudentAdmission(String courseName, String studRollNo) {

        log.info("Cancelling admission for studRollNo={} from course={}", studRollNo, courseName);
    ...
    @Override
    public Student updateStudentRecordByRollNo(String rollNumber, Student updatedStudent) {

        log.info("Updating student record for student rollNo: {}", rollNumber);
